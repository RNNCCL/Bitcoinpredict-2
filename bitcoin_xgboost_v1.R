...
BTClog <- c(BTClog, xts(as.numeric(735.90), Sys.Date()))
...
volume_total <- Quandl("BCHAIN/NTRAT", start_date = "2011-11-24", order="asc") 

volume <- as.data.frame(volume_total$Value[1:1828])

colnames(volume) <- "volume"

#revenue
revenue_total <- Quandl("BCHAIN/MIREV", start_date = "2011-11-24", order="asc") 

revenue <- as.data.frame(revenue_total$Value[1:1828])

colnames(revenue) <- "revenue"


#number of transactions per day
transactions_total <- Quandl("BCHAIN/NTRAN", start_date = "2011-11-24", order="asc") 

transactions <- as.data.frame(transactions_total$Value[1:1828])

colnames(transactions) <- "transactions"


my_data <- as.data.frame(cbind(my_data, volume, revenue, transactions))
...
my_data <- as.data.frame(cbind(my_data, OBV_BTC, mfi5, mfi10, mfi20, mfi40, mfi60, mfi100))



### offsetting by one day

predict_value <- BTCUSD[2:1827]

colnames(predict_value) <- "BTC_next_day"

total_data <- cbind(my_data[1:1826,], as.data.frame(predict_value))
...
modelv1 <- lm( BTC_next_day ~. -BTC.USD , data = total_data )
...
############ building xgboost model #################3



train_set_v2 <- data.matrix(train_set[,(names(train_set) %in% relevant_features)])


set.seed(1)
'''
xgb <- xgboost(data = matrix_data,
label = load_data$diff_offset_1,
eta = 0.3, #default 0.3
...
require(xgboost)
  ...
  history <- xgb.cv(
                  nfold = 5,
                  data = train_set_v2,
                  label = train_set$BTC_next_day,
                  eta = 0.1, #default 0.3
                  max_depth = 10, #default 6
                  subsample = 1,
                  #colsample_bytree = round(sqrt(length(total_data))/length(total_data),2),
                  eval_metric = "mae",
                  objective = "reg:linear",
                  nrounds = 1500
)
...
### Cross validation:
 ...
 my_predict_1 <- predict(xgb_1, data.matrix(test_set[,(names(train_set) %in% relevant_features)]))  
  
  
 results <- as.data.frame(cbind(my_predict_1, test_set$BTC_next_day))  
 ...
 #plot(results$error_total_rounded,results$error)
 ...
 ##################################################################
 ################## Predicting price next day #####################
 ##################################################################
  

xgb1 <-  xgboost(
  #nfold = 5,
  data = data.matrix(matrix_data[,(names(matrix_data) %in% relevant_features)]),
  label = matrix_data$BTC_next_day,
  eta = 0.1, #default 0.3
  max_depth = 10, #default 6
  subsample = 1,
  #colsample_bytree = round(sqrt(length(total_data))/length(total_data),2),
 ...
 pred_tomorrow_xgb <- predict(xgb1, data.matrix(my_data[1828,(names(my_data) %in% relevant_features)]))
 pred_tomorrow_xgb <- predict(xgb2, data.matrix(my_data[1828,(names(my_data) %in% relevant_features)]))
 pred_tomorrow_xgb <- predict(xgb3, data.matrix(my_data[1828,(names(my_data) %in% relevant_features)]))
 pred_tomorrow_xgb <- predict(xgb4, data.matrix(my_data[1828,(names(my_data) %in% relevant_features)]))
