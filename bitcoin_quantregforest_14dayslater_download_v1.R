...
predict_value <- BTCUSD[15:1827]

colnames(predict_value) <- "BTC_next_day"

total_data <- cbind(my_data[1:1813,], as.data.frame(predict_value))
...
pred_14day_quant <- predict(qf_final, newdata = my_data[1828,], what = c(0.01, 0.025, 0.05,  0.1, 0.25, 0.5,0.75, 0.95, 0.975, 0.99))
pred_14day_mean <- predict(qf_final, newdata = my_data[1828,], what = mean)


pred_14day_quant
pred_14day_mean

pred_14_day_cdf <- predict(qf_final, newdata = my_data[1828,], what = seq(0.01, 1, 0.0001))
#plot(seq(0.01, 1, 0.0001),pred_14_day_cdf )

#hist(pred_14_day_cdf, 1000 )

plot(density(pred_14_day_cdf))
plot(ecdf(pred_14_day_cdf ))
