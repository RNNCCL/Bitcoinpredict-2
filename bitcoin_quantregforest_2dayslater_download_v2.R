...
predict_value <- BTCUSD[3:1827]

colnames(predict_value) <- "BTC_next_day"

total_data <- cbind(my_data[1:1825,], as.data.frame(predict_value))
...
pred_twoday_quant <- predict(qf_final, newdata = my_data[1828,], what = c(0.01, 0.025, 0.1, 0.25, 0.5,0.75, 0.95, 0.975, 0.99))
pred_twoday_mean <- predict(qf_final, newdata = my_data[1828,], what = mean)


pred_twoday_quant
pred_twoday_mean

pred_two_day_cdf <- predict(qf_final, newdata = my_data[1828,], what = seq(0.01, 1, 0.001))
plot(seq(0.01, 1, 0.001),pred_two_day_cdf )
