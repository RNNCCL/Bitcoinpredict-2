...
BTClog <- c(BTClog, xts(as.numeric(758.30), as.Date("2016-12-06")))
...
days <- as.data.frame(as.factor(weekdays(as.Date(index(BTCbb)))))

colnames(days) <- "days"

BTClog$days <- days$days
...
volume_total <- Quandl("BCHAIN/NTRAT", start_date = "2011-12-06", order="asc") 
...
#revenue
revenue_total <- Quandl("BCHAIN/MIREV", start_date = "2011-12-06", order="asc") 
...
transactions_total <- Quandl("BCHAIN/NTRAN", start_date = "2011-12-06", order="asc") 
...
pred_oneday_quant <- predict(qf_final, newdata = my_data[1828,], what = c(0.01, 0.025, 0.1, 0.25, 0.5,0.75, 0.95, 0.975, 0.99))
pred_oneday_mean <- predict(qf_final, newdata = my_data[1828,], what = mean)


pred_oneday_quant
pred_oneday_mean

pred_one_day_cdf <- predict(qf_final, newdata = my_data[1828,], what = seq(0.01, 1, 0.001))
#plot(seq(0.01, 1, 0.001),pred_one_day_cdf )

#hist(pred_one_day_cdf, 200 )
#hist(pred_one_day_cdf, 1000 )

plot(density(pred_one_day_cdf))
plot(ecdf(pred_one_day_cdf ))
