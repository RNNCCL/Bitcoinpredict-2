...
#revenue
revenue_total <- Quandl("BCHAIN/MIREV", start_date = "2011-11-23", order="asc") 
...
transactions_total <- Quandl("BCHAIN/NTRAN", start_date = "2011-11-23", order="asc") 
...
ridge_1 <- lm.ridge(train_set$BTC_next_day ~. , train_set[,(names(train_set) %in% relevant_features)], lambda=seq(0,20,0.1))

my_lm_fit <- predict.lm(ridge_1, test_set[,(names(train_set) %in% relevant_features)])

my_predict = scale(test_set[,(names(train_set) %in% relevant_features)],center = F, scale = ridge_1$scales)%*%ridge_1 $coef[,which.min(ridge_1$GCV)] + ridge_1$ym
#summary((y.pred.ridge - test_set$BTC_next_day)^2)


results <- as.data.frame(cbind(my_predict, test_set$BTC_next_day))
